function Invoke-WinUtilAssets {
  ...
  if ($type -eq 'logo') {
      $asset = New-WinUtilBecamexLogo -Size $Size -Render:$render

      if ($render -and $null -ne $sync -and $sync.ContainsKey("RenderedAssetCache")) {
          $sync.RenderedAssetCache[$cacheKey] = $asset
      }
